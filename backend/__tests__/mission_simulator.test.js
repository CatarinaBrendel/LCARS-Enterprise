...
const SEED = [
  // code,        status(DB),     sector,  authority,              (progress computed via objectives)
  ['LIST-ALPHA',  'planned',      'Alpha', 'Starfleet Ops'],
  ['LIST-BETA',   'in_progress',  'Beta',  'USS Voyager'],
  ['LIST-GAMMA',  'completed',    'Gamma', 'Federation Science'],
];
...
beforeAll(async () => {
  ...
  // Insert 3 missions with different statuses/sectors
  for (const [code, status, sector, authority] of SEED) {
    const m = await query(
    ...
    // Give LIST-BETA an objective whose title we can search for
    if (code === 'LIST-BETA') {
      await query(
    ...
    // Add a created event just to have timeline rows
    await query(
...
afterAll(async () => {
  // cleanup the exact rows we created, in FK-safe order
  try {
...
test('GET /api/missions filters by status (UI labels) and scopes by q=LIST-', async () => {
  const res = await request(app)
    .get('/api/missions')
    .query({
      status: 'IN PROGRESS,DONE', // UI labels (mapped server-side)
      q: 'LIST-'                  // limit to our seed missions
    });
...
test('GET /api/missions supports pagination', async () => {
  ...
  // total should reflect all our LIST-* missions (>= 3)
