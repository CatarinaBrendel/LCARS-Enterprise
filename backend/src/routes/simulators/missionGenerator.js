...
export async function createRandomMission(
  ...
  const {
    ...
    stardate: stardateArg,
    withTeams = true,
  } = opts;

  const {emitMissionCreated} = emitters;
  const sector = sectorArg ?? pick(SECTORS);
  const authority = authorityArg ?? pick(AUTHORITIES);
  const stardate = stardateArg ?? (Math.round(Date.now() / 86_400_000) + 41000);
  let mission;
  let insertedObj = [];
  let teams = [];
  
  const minObj = Math.max(1, Math.floor(objectivesMin));
  const maxObj = Math.max(minObj, Math.floor(objectivesMax));
  ...
  try {
    ...
    // 2) objectives
    const howMany = rand(minObj, maxObj);
    const chosen = sample(OBJ_TITLES, howMany);
    // safety: if something weird makes chosen empty, seed a fallback
    const finalTitles = chosen.length ? chosen : ['Primary objective'];
    for (let i = 0; i < finalTitles.length; i++) {
      const title = finalTitles[i];
      const details = 'Autogenerated objective';
      const priority = finalTitles.length - i;
  ...
  if (emitMissionCreated && mission?.id) {
    try {
      console.log('[missionGen] calling emitMissionCreated with id', mission.id);
      await emitMissionCreated(mission.id);
      
    } catch (err) {
      console.error('[missionGen] emitMissionCreated failed:', err);
    }
  } else {
    console.warn('[missionGen] emitMissionCreated is not provided — no WS broadcast');
   }
...
export function startMissionGenerator({
  ...
  console.log('[missionGen] start: typeof emitMissionCreated =', typeof emitMissionCreated);
  ...
  const tick = async () => {
    try {
      ...
      logger.error('[missionGen] create mission error:', err?.stack || err);
    } finally {
      scheduleNext();
    }
  };
