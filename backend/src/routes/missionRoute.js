...
// POST /api/missions/generate?count=5
router.post("/generate", async (req, res) => {
  const n = Math.max(1, Math.min(25, Number(req.query.count ?? 5)));
  const created = [];

  for (let i = 0; i < n; i++) {
    const code = `MSN-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
    const sector = ["Alpha", "Beta", "Gamma", "Delta", "Tau"][Math.floor(Math.random()*5)];
    const authority = ["Starfleet Command", "Science Council", "Colony Admin", "Diplomatic Corps"][Math.floor(Math.random()*4)];

    const { rows: m } = await query(
      `INSERT INTO mission (code, sector, authority, status, progress_pct, created_at, updated_at)
        VALUES ($1,$2,$3,'planned',0,NOW(),NOW())
        RETURNING id, code`,
    ...
    const titles = ["Scan anomaly", "Rescue survivors", "Repair relay", "Map corridor", "Deliver supplies"];
    ...
    for (let j = 0; j < howMany; j++) {
      await query(
        ...
        [missionId, titles[(j + Math.floor(Math.random()*titles.length)) % titles.length], "Autogenerated", howMany - j]
      );
    }

    // ✨ Tell clients there is a new mission now
    const { emitMissionCreated } = req.app.get('ws') || {};
    emitMissionCreated?.(missionId);


    created.push({ id: missionId, code: m[0].code });
  }
