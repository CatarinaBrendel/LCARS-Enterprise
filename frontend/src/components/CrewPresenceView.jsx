...
export default function CrewPresenceView() {
  const { list, onDuty, busy, summary } = usePresence();
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card title="On Duty">
          <span className="text-3xl font-bold">{summary.onDuty}</span>
          <span className="opacity-70 ml-1">/ {summary.total}</span>
        </Card>
        <Card title="Busy Now">
          <span className="text-3xl font-bold">{summary.busy}</span>
          <span className="opacity-70 ml-1">({summary.busyPct}% )</span>
        </Card>
        <Card title="Ticker">
          <div className="overflow-x-hidden whitespace-nowrap">
            <div className="animate-[marquee_18s_linear_infinite]">
              {onDuty.map(c => c.name || `#${c.crewId}`).join('   •   ')}
            </div>
          </div>
        </Card>
      </div>
          ...
          <tbody className="text-[#e8e2d0]">
            {list.map(row => (
              <tr key={row.crewId} className="border-t border-[#f2a007]/20 [&>td]:py-2 [&>td]:px-2">
                ...
                <td className="flex items-center gap-2"><Dot on={!!row.busy} /><span className="opacity-80">{row.busy ? 'Busy' : 'Idle'}</span></td>
                <td>{row.deck_zone || '—'}</td>
